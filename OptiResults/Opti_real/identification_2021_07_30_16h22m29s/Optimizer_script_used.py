...
log_path_real=os.path.join("/home/mehdi/tmp/logs/log_4_2021-7-20-10-58-16/log_real.csv")     

true_params_path=os.path.join(log_dir_path,"params.json")
name_script='Optimizer_script_2.py'
log_real=True
...
data_prepared_train,data_prepared_test=train_test_split(data_prepared,test_size=0.9, random_state=41)
...
if spath is not None:
    with open(os.path.join(spath,"Params_initiaux.csv"),'w') as f:
        ...
        for keys in current_Dict_variables.keys():
            sdict[keys]=current_Dict_variables[keys]
